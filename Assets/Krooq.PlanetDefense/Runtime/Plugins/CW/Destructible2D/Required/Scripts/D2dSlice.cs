	...
	public static class D2dSlice
	{
		...
		public static void ForceAll(Vector2 startPos, Vector2 endPos, float thickness, float force, ForceMode2D forceMode, int layerMask = -1)
		{
			...
			var resultCount = default(int);
				...
				var filter = new ContactFilter2D(); filter.SetLayerMask(layerMask);

				resultCount = Physics2D.Linecast(startPos - right, endPos - right, filter, results); // Left side

				Force(resultCount, -force2, forceMode);

				resultCount = Physics2D.Linecast(startPos + right, endPos + right, filter, results); // Right side

				Force(resultCount, force2, forceMode);
			#else
				resultCount = Physics2D.LinecastNonAlloc(startPos - right, endPos - right, results, layerMask); // Left side

				Force(resultCount, -force2, forceMode);

				resultCount = Physics2D.LinecastNonAlloc(startPos + right, endPos + right, results, layerMask); // Right side
