...
namespace Destructible2D
{
	...
	public class D2dEdgeCollider : D2dCollider
	{
		...
		private void Refresh(EdgeCollider2D collider)
		{
			...
			if (collider.usedByEffector != usedByEffector) collider.usedByEffector = usedByEffector;

			#if UNITY_6000_0_OR_NEWER
				if (collider.compositeOperation != compositeOperation) collider.compositeOperation = compositeOperation;
			#else
				if (collider.usedByComposite != usedByComposite) collider.usedByComposite = usedByComposite;
			#endif
