	...
	public static class D2dContext
	{
		...
		private static void MakeDestructibleDetSpl(MenuCommand menuCommand)
		{
			...
			AddSingleComponent<D2dPolygonCollider>(gameObject);
				...
				AddSingleComponent<Rigidbody2D>(gameObject, r => r.bodyType = RigidbodyType2D.Kinematic);
			#else
			...
			AddSingleComponent<D2dMakeRigidbody2DDynamic>(gameObject, d =>
				{
					d.enabled = false;
				});
			AddSingleComponent<D2dRequirements>(gameObject, r =>
			...
			AddSingleComponent<D2dFixtureGroup>(gameObject, r =>
				{
					var enableMethod   = typeof(D2dMakeRigidbody2DDynamic).GetProperty("enabled").GetSetMethod();
					var enableDelegate = (UnityAction<bool>)System.Delegate.CreateDelegate(typeof(UnityAction<bool>), r.GetComponent<D2dMakeRigidbody2DDynamic>(), enableMethod);

					UnityEditor.Events.UnityEventTools.AddBoolPersistentListener(r.OnAllDetached, enableDelegate, false);
