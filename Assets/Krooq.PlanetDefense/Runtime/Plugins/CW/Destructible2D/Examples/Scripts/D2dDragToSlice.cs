...
namespace Destructible2D.Examples
{
	...
	public class D2dDragToSlice : MonoBehaviour, UnityEngine.ISerializationCallbackReceiver
	{
		...
		/// <summary>This component stretches the <b>StampShape</b> between the slice points. This can cause the slice edges to be distorted, so you can add caps to maintain their shape.
		...
		protected virtual void Update()
		{
			...
			for (var i = links.Count - 1; i >= 0; i--)
			{
				...
				if (Controls.WentUp(link.Finger, true) == true)
				{
					...
					DoSliceAll(link.Start, position);
...
namespace Destructible2D.Examples
{
	...
	public class D2dDragToSlice_Editor : CwEditor
	{
		protected override void OnInspector()
		{
			...
			Draw("CapShape", "This component stretches the <b>StampShape</b> between the slice points. This can cause the slice edges to be distorted, so you can add caps to maintain their shape.\n\nNOTE: If you use caps, then the <b>StampShape</b> should be 1px high or stretched vertically, and the cap height should be a half the width and be rounded at the top.");
